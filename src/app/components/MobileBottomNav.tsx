    ...
    <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40 border-t border-[rgb(var(--color-surface-border-rgb))] bg-[rgb(var(--color-surface-alt-rgb))] backdrop-blur-md" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
      <div className="mx-auto md:max-w-xl">
        <div className="grid grid-cols-3 gap-0 p-0">
          ...
          <button onClick={onSelectExtractor} className={`group relative flex items-center justify-center px-3 py-3 transition-all md:rounded-xl ${active === 'extractor' ? 'text-[rgb(var(--color-primary-rgb))] bg-[rgba(var(--color-primary-rgb),0.12)]' : 'text-[rgb(var(--color-on-surface-muted-rgb))] hover:bg-[rgb(var(--color-surface-border-rgb))]'}`} aria-label="Tag Extractor" aria-current={active === 'extractor' ? 'page' : undefined}>
            <TagIcon />
          ...
          <button onClick={onSelectImage} className={`group relative flex items-center justify-center px-3 py-3 transition-all md:rounded-xl ${active === 'image' ? 'text-[rgb(var(--color-primary-rgb))] bg-[rgba(var(--color-primary-rgb),0.12)]' : 'text-[rgb(var(--color-on-surface-muted-rgb))] hover:bg-[rgb(var(--color-surface-border-rgb))]'}`} aria-label="Image Metadata" aria-current={active === 'image' ? 'page' : undefined}>
            <PhotoIcon className="h-5 w-5" />
            <span className="ml-2 text-xs font-medium">Image</span>
            <span className="pointer-events-none absolute inset-0 rounded-xl bg-current opacity-0 group-active:opacity-10 transition-opacity" />
          </button>
          <button onClick={onOpenSettings} className={`group relative flex items-center justify-center px-3 py-3 transition-all md:rounded-xl ${highlightSettings ? 'text-[rgb(var(--color-primary-rgb))] bg-[rgba(var(--color-primary-rgb),0.12)]' : 'text-[rgb(var(--color-on-surface-muted-rgb))] hover:bg-[rgb(var(--color-surface-border-rgb))]'}`} aria-label="Settings">
            <CogIcon />
