    ...
    >
      ...
      >
        <div className="rounded-xl bg-[rgb(var(--color-error-bg-rgb))] border border-[rgb(var(--color-error-rgb))]/20 p-4 md:p-6">
          ...
          {maxRetryReached && (
            <p className="text-xs md:text-sm text-[rgb(var(--color-error-rgb))] font-semibold mt-3">
      ...
      {/* What is Booru Tag Extractor - Only show when showFullInfo is true */}
      {showFullInfo && (
        <motion.div
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
      ...
      >
        {/* Retry Button - Show "Retry Again" if max retry reached, otherwise normal Retry */}
        {maxRetryReached ? (
          <button
