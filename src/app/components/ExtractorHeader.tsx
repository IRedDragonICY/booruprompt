...
export const ExtractorHeader: React.FC<ExtractorHeaderProps> = ({ activeSite, url, onUrlChange, onExtract, onReset, loading }) => {
  return (
    <div className="shrink-0 border-b border-[rgb(var(--color-surface-border-rgb))] bg-[rgb(var(--color-surface-alt-rgb))] px-4 py-3">
      <div className="flex items-center gap-2">
        <h1 className="text-base font-semibold sm:text-lg">Booru Tag Extractor</h1>
      ...
      <div className="mt-2 hidden md:block">
        <span className="mr-2 text-xs text-[rgb(var(--color-on-surface-muted-rgb))]">Supports:</span>
        {BOORU_SITES.map(s => (
          <span
            key={s.name}
            className={`mb-1 mr-1 inline-block rounded-full px-2 py-0.5 text-xs font-medium transition-colors duration-150 ${
              activeSite === s.name
