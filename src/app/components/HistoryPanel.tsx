...
export interface BaseHistoryPanelProps<T extends { id: string; timestamp: number }> {
  ...
  onClearHistory: () => void;
  noToggle?: boolean; // render body directly without collapsible header
...
export function HistoryPanelBase<T extends { id: string; timestamp: number }>({ title, history, renderItem, filterPredicate, searchPlaceholder, onClearHistory, noToggle = false, className = '' }: BaseHistoryPanelProps<T>) {
  ...
  const normalizedTitle = (title || t('extractor.history.searchFallback')).toLowerCase().replace(/\s+/g, '-');
    ...
    <>
      <div className="p-4">
        <div className="relative">
          <input type="text" placeholder={searchPlaceholder} value={searchQuery} onChange={handleSearchChange} className="w-full rounded-lg border border-[rgb(var(--color-surface-border-rgb))] bg-[rgb(var(--color-surface-alt-2-rgb))] py-2 pl-10 pr-10 text-sm text-[rgb(var(--color-on-surface-rgb))] placeholder:text-[rgb(var(--color-on-surface-faint-rgb))] transition focus:border-[rgb(var(--color-primary-rgb))] focus:outline-none focus:ring-1 focus:ring-[rgb(var(--color-primary-rgb))]" aria-label={t('extractor.history.searchAriaLabel', { context: title || t('extractor.history.searchFallback') })}/>
          ...
          {searchQuery && (<TooltipWrapper tipContent={t('extractor.history.clearSearchTooltip')}><motion.button whileTap={{ scale: 0.9 }} onClick={handleClearSearch} className="absolute right-2 top-1/2 -translate-y-1/2 transform rounded-full p-1 text-[rgb(var(--color-on-surface-faint-rgb))] transition-colors hover:bg-[rgb(var(--color-surface-border-rgb))] hover:text-[rgb(var(--color-on-surface-rgb))]" aria-label={t('extractor.history.clearSearchAria')} initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.5 }}><XMarkIcon className="h-4 w-4"/></motion.button></TooltipWrapper>)}
        </div>
      </div>
      <div className="max-h-80 space-y-2 overflow-y-auto px-4 pb-4 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-[rgb(var(--color-surface-border-rgb))]">
        ...
        {filteredHistory.length === 0 && <p className="py-4 text-center text-sm text-[rgb(var(--color-on-surface-muted-rgb))]">{t('extractor.history.emptySearch')}</p>}
      </div>
      <div className="relative border-t border-[rgb(var(--color-surface-border-rgb))] bg-[rgb(var(--color-surface-alt-2-rgb))] p-3 text-right">
        <AnimatePresence>{showClearConfirm && (<motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 5 }} className="absolute right-3 bottom-full z-30 mb-2 flex items-center gap-2 rounded-sm border border-[rgb(var(--color-surface-border-rgb))] bg-[rgb(var(--color-surface-rgb))] p-2 shadow-lg"><p className="text-xs text-[rgb(var(--color-on-surface-rgb))]">{t('extractor.history.confirmMessage')}</p><motion.button whileTap={{ scale: 0.95 }} onClick={handleClearClick} className="rounded-sm bg-red-500 px-2 py-1 text-xs font-medium text-white transition hover:bg-red-600">{t('extractor.history.confirmYes')}</motion.button><motion.button whileTap={{ scale: 0.95 }} onClick={handleHideConfirm} className="rounded-sm bg-[rgb(var(--color-surface-border-rgb))] px-2 py-1 text-xs font-medium text-[rgb(var(--color-on-surface-rgb))] transition hover:bg-gray-300 dark:hover:bg-gray-500">{t('extractor.history.confirmCancel')}</motion.button></motion.div>)}</AnimatePresence>
        <TooltipWrapper tipContent={t('extractor.history.clearTooltip')}><motion.button whileTap={{ scale: 0.95 }} onClick={handleShowConfirm} className="inline-flex items-center space-x-1 rounded-md bg-[rgb(var(--color-error-bg-rgb))] px-2.5 py-1 text-xs font-medium text-[rgb(var(--color-error-rgb))] transition hover:bg-red-100 dark:hover:bg-red-900/50" aria-label={`${t('extractor.history.clearAction')} ${title || t('extractor.history.searchFallback')}`}><AnimatedIcon animation="gentle"><TrashIcon /></AnimatedIcon><span>{t('extractor.history.clearAction')}</span></motion.button></TooltipWrapper>
      </div>
    </>
  );
  ...
  return (
    <div className={`mt-6 overflow-hidden rounded-lg border border-[rgb(var(--color-surface-border-rgb))] bg-[rgb(var(--color-surface-alt-rgb))] ${className}`}>
      ...
      <AnimatePresence initial={false}>
        {isOpen && (
          <motion.div id={`${normalizedTitle}-content`} key="content" initial="collapsed" animate="open" exit="collapsed" variants={{ open: { opacity: 1, height: "auto" }, collapsed: { opacity: 0, height: 0 } }} transition={{ duration: 0.3, ease: [0.04, 0.62, 0.23, 0.98] }} className="overflow-hidden">
            {Body}
          </motion.div>
        )}
      </AnimatePresence>
