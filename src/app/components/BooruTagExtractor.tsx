...
const utils = {
    getCategoryFromClassList: (element: Element): TagCategory => {
        ...
        const parentCategory = element.closest('[class*="tag-type-"]');
        if (parentCategory) return utils.getCategoryFromClassList(parentCategory);
        return 'general';
    },
    ...
    extractTagsByClass: (doc: Document, selectors: { container: string, tag: string }): ExtractedTag[] => {
        ...
        elements.forEach(element => {
            const tagElement = element.querySelector(selectors.tag) ?? element;
...
const extractionStrategies = {
    ...
    safebooru: (doc: Document): ExtractionResult => ({
        tags: utils.extractTagsBySection(doc, '#tag-sidebar', 'a[href*="page=post"]'),
        imageUrl: utils.extractImageUrl(doc, '#image', 'src'),
        title: utils.extractTitle(doc, 'title')
    }),
...
const BOORU_SITES = [
    ...
    { name: 'Safebooru', urlPattern: /safebooru\.org\/index\.php\?page=post&s=view&id=\d+/i, extractTags: extractionStrategies.safebooru },
        ...
        <div className="relative w-full h-60 group">
            ...
            <div className="relative w-full h-full overflow-hidden rounded-md bg-gray-800">
                ...
                {title && !imgLoading && !imgError && (
                    <div className="absolute bottom-0 left-0 right-0 bg-black bg-opacity-70 text-white p-2 text-sm opacity-0 group-hover:opacity-100 transition-opacity duration-300 truncate">
                        {title}
                    </div>
                )}
            </div>
...
const BooruTagExtractor = () => {
    ...
    const extractTags = useCallback(async (targetUrl: string) => {
        ...
        console.log(`Attempting extraction for: ${targetUrl}`);
        setLoading(true);
        setError('');
        setAllExtractedTags([]);
        setImageUrl(undefined);
        ...
        try {
            ...
            if (doc.title.includes("error") || doc.body.textContent?.includes("Could not retrieve contents")) {
    ...
    useEffect(() => {
        ...
        if (url && url.includes('.') && url.length > 10) {
            const site = BOORU_SITES.find(s => s.urlPattern.test(url));
            if (site) {
                debounceTimeoutRef.current = setTimeout(() => {
                    if (url !== currentExtractionUrl.current) {
                        extractTags(url).catch(error => {
                            console.error("Extraction failed in debounce handler:", error);
                        });
                    }
                }, 750);
            } else {
        ...
        } else if (!url && currentExtractionUrl.current) {
            handleReset();
        }
    ...
    const handleUrlChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setUrl(newUrl);
        if (error && newUrl !== currentExtractionUrl.current) {
            setError('');
        }
    };
                ...
                <div className="bg-gradient-to-r from-gray-800 to-gray-700 p-6 relative overflow-hidden border-b border-gray-700">
                    ...
                    <p className="text-sm text-gray-300 mt-1 relative z-10 animate-fadeSlideDown animation-delay-100 flex flex-wrap gap-x-2">
                        ...
                        {BOORU_SITES.map((site, index) => (
                            <span key={site.name} className={`${activeSite === site.name ? 'text-blue-300 font-medium' : ''}`}>
                ...
                <div className="p-6 space-y-6 animate-fadeIn animation-delay-200">
                    <div className="transform transition-all duration-300 hover:scale-[1.01]">
                        <label htmlFor="url" className="block text-sm font-medium text-gray-300 mb-2">
                            Booru Post URL (e.g., Danbooru, Safebooru, Gelbooru...)
                        </label>
                        <input
                            id="url"
                            type="url"
                    ...
                    <div className="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3 animate-fadeSlideUp animation-delay-300">
                        <button
                            ...
                            className="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-medium py-3 px-4 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-gray-800 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 transform hover:enabled:scale-105 active:enabled:scale-95 hover:enabled:shadow-lg"
