...
export const SettingsPanel = memo(function SettingsPanel({ settings, onSettingsChange }: SettingsPanelProps) {
    ...
    const DEFAULT_BLACKLIST_KEYWORDS = t('defaultBlacklistKeywords');
    ...
    const handleBlacklistReset = useCallback(() => { setLocalBlacklist(DEFAULT_BLACKLIST_KEYWORDS); onSettingsChange({ blacklistKeywords: DEFAULT_BLACKLIST_KEYWORDS }); }, [onSettingsChange, DEFAULT_BLACKLIST_KEYWORDS]);
    ...
    const HISTORY_SIZE_OPTIONS = useMemo(() => [
        { label: t('settings.maxHistorySize.options.10'), value: 10 },
        { label: t('settings.maxHistorySize.options.30'), value: 30 },
        { label: t('settings.maxHistorySize.options.50'), value: 50 },
        { label: t('settings.maxHistorySize.options.100'), value: 100 },
        { label: t('settings.maxHistorySize.options.unlimited'), value: -1 },
    ], [t]);

    const themeOptions = useMemo(() => [
        { value: 'system' as ThemePreference, label: t('settings.themes.system'), icon: <ComputerDesktopIcon />, animation: 'gentle' as const },
        { value: 'light' as ThemePreference, label: t('settings.themes.light'), icon: <SunIcon />, animation: 'spin' as const },
        { value: 'dark' as ThemePreference, label: t('settings.themes.dark'), icon: <MoonIcon />, animation: 'default' as const },
    ], [t]);
    const colorThemeOptions = useMemo(() => [
        { value: 'blue' as ColorTheme, label: t('settings.colors.blue'), colorClass: 'bg-[#3B82F6] dark:bg-[#60A5FA]' },
        { value: 'orange' as ColorTheme, label: t('settings.colors.orange'), colorClass: 'bg-[#F97316] dark:bg-[#FB923C]' },
        { value: 'teal' as ColorTheme, label: t('settings.colors.teal'), colorClass: 'bg-[#0D9488] dark:bg-[#2DD4BF]' },
        { value: 'rose' as ColorTheme, label: t('settings.colors.rose'), colorClass: 'bg-[#E11D48] dark:bg-[#FB7185]' },
        { value: 'purple' as ColorTheme, label: t('settings.colors.purple'), colorClass: 'bg-[#8B5CF6] dark:bg-[#A78BFA]' },
        { value: 'green' as ColorTheme, label: t('settings.colors.green'), colorClass: 'bg-[#16A34A] dark:bg-[#4ADE80]' },
    ], [t]);
    const fetchModeOptions = useMemo(() => [
        { value: 'server' as FetchMode, label: t('settings.fetchModes.server'), icon: <ServerIcon />, description: t('settings.fetchModes.serverDesc') },
        { value: 'clientProxy' as FetchMode, label: t('settings.fetchModes.client'), icon: <CloudArrowDownIcon />, description: t('settings.fetchModes.clientDesc') },
    ], [t]);
    ...
    return (
        <div className="flex flex-col flex-1 overflow-hidden">
            ...
            <div className="flex-grow overflow-y-auto p-6 space-y-6 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-[rgb(var(--color-surface-border-rgb))]">
                <div>
                    <label className="mb-2 flex items-center text-sm font-medium text-[rgb(var(--color-on-surface-rgb))]"><span className="mr-2"><PaletteIcon /></span><span>{t('settings.appearance')}</span></label>
                ...
                <div>
                    <label className="mb-2 flex items-center text-sm font-medium text-[rgb(var(--color-on-surface-rgb))]"><span className="mr-2"><PaletteIcon /></span><span>{t('settings.colorTheme')}</span></label>
                    <div className="grid grid-cols-3 gap-2 rounded-xl bg-[rgb(var(--color-surface-alt-2-rgb))] p-2">
                        ...
                        <TooltipWrapper tipContent={t('settings.colors.custom')}>
                ...
                <div>
                    <label className="mb-2 flex items-center text-sm font-medium text-[rgb(var(--color-on-surface-rgb))]"><span>{t('settings.dataFetching')}</span></label>
                            ...
                            <div key={value}>
                                ...
                                {value === 'clientProxy' && settings.fetchMode === 'clientProxy' && (
                                    <motion.div initial={{ opacity: 0, height: 0, marginTop: 0 }} animate={{ opacity: 1, height: 'auto', marginTop: '0.5rem' }} exit={{ opacity: 0, height: 0, marginTop: 0 }} transition={{ duration: 0.2 }} className="pl-12 pr-3">
                                        <label htmlFor="clientProxySelect" className="mb-1 block text-xs font-medium text-[rgb(var(--color-on-surface-muted-rgb))]">{t('settings.selectClientProxy')}</label>
                                        ...
                                        <p className="mt-1 text-[10px] text-[rgb(var(--color-on-surface-faint-rgb))]">{t('settings.proxyNote')}</p>
                                    </motion.div>
                                )}
                            </div>
                        ))}
                ...
                <div className="rounded-xl border border-[rgb(var(--color-surface-border-rgb))] bg-[rgb(var(--color-surface-rgb))] p-3">
                    <label className="flex cursor-pointer select-none items-center justify-between">
                        <TooltipWrapper tipContent={t('settings.autoExtract.desc')}><span className="mr-3 text-sm font-medium text-[rgb(var(--color-on-surface-rgb))] flex items-center"><AutomaticIcon /><span className="ml-2">{t('settings.autoExtract.title')}</span></span></TooltipWrapper>
                    ...
                    <p className="mt-1.5 text-xs text-[rgb(var(--color-on-surface-muted-rgb))]">{t('settings.autoExtract.label')}</p>
                </div>

                <div className="rounded-xl border border-[rgb(var(--color-surface-border-rgb))] bg-[rgb(var(--color-surface-rgb))] p-3">
                    <label className="flex cursor-pointer select-none items-center justify-between"><TooltipWrapper tipContent={t('settings.imagePreviews.desc')}><span className="mr-3 text-sm font-medium text-[rgb(var(--color-on-surface-rgb))] flex items-center"><PreviewIcon /><span className="ml-2">{t('settings.imagePreviews.title')}</span></span></TooltipWrapper>
                    ...
                    <p className="mt-1.5 text-xs text-[rgb(var(--color-on-surface-muted-rgb))]">{t('settings.imagePreviews.label')}</p>
                </div>

                <div className="rounded-xl border border-[rgb(var(--color-surface-border-rgb))] bg-[rgb(var(--color-surface-rgb))] p-3">
                    <label className="flex cursor-pointer select-none items-center justify-between"><TooltipWrapper tipContent={t('settings.saveHistory.desc')}><span className="mr-3 text-sm font-medium text-[rgb(var(--color-on-surface-rgb))] flex items-center"><HistorySaveIcon /><span className="ml-2">{t('settings.saveHistory.title')}</span></span></TooltipWrapper>
                    ...
                    <p className="mt-1.5 text-xs text-[rgb(var(--color-on-surface-muted-rgb))]">{t('settings.saveHistory.label')}</p>
                </div>

                <div className="rounded-xl border border-[rgb(var(--color-surface-border-rgb))] bg-[rgb(var(--color-surface-rgb))] p-3">
                    <label className="flex cursor-pointer select-none items-center justify-between"><TooltipWrapper tipContent={t('settings.unsupportedSites.desc')}><span className="mr-3 text-sm font-medium text-[rgb(var(--color-on-surface-rgb))] flex items-center"><UnsupportedSitesIcon /><span className="ml-2">{t('settings.unsupportedSites.title')}</span></span></TooltipWrapper>
                ...
                <div className="rounded-xl border border-[rgb(var(--color-surface-border-rgb))] bg-[rgb(var(--color-surface-rgb))] p-3">
                    ...
                    <p className="mt-1.5 text-xs text-[rgb(var(--color-on-surface-muted-rgb))]">{t('settings.blacklist.placeholder')}</p>
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: settings.enableBlacklist ? 1 : 0, height: settings.enableBlacklist ? 'auto' : 0 }} className={`mt-2 space-y-2 overflow-hidden ${settings.enableBlacklist ? '' : 'pointer-events-none'}`}>
                        ...
                        <div className="text-right"><button type="button" onClick={handleBlacklistReset} className="rounded-md bg-[rgb(var(--color-surface-border-rgb))] px-3 py-1.5 text-xs font-medium text-[rgb(var(--color-on-surface-muted-rgb))] transition hover:bg-gray-300 dark:hover:bg-gray-500 active:scale-95">{t('settings.blacklist.reset')}</button></div>
                    </motion.div>
                </div>
                ...
                <AnimatePresence>
                    {settings.saveHistory && (
                        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.2 }}>
                            <label htmlFor="maxHistorySizeSelect" className="mb-1.5 text-sm font-medium text-[rgb(var(--color-on-surface-rgb))] flex items-center"><HistorySizeIcon /><span className="ml-2">{t('settings.maxHistorySize.title')}</span></label>
                            <div className="relative">
                            ...
                            <p className="mt-1.5 text-xs text-[rgb(var(--color-on-surface-muted-rgb))]">{t('settings.maxHistorySize.desc')}</p>
                        </motion.div>
                    )}
                </AnimatePresence>

                <div className="space-y-3 border-t border-[rgb(var(--color-surface-border-rgb))] pt-4">
                    <label className="block text-sm font-medium text-[rgb(var(--color-on-surface-rgb))]">{t('settings.language.title')}</label>
                    <p className="text-xs text-[rgb(var(--color-on-surface-muted-rgb))]">{t('settings.language.desc')}</p>
                ...
                <div className="space-y-3 border-t border-[rgb(var(--color-surface-border-rgb))] pt-4">
                    ...
                    <a href={REPORT_ISSUE_URL} target="_blank" rel="noopener noreferrer" className="inline-flex w-full items-center justify-center space-x-2 rounded-lg border border-[rgb(var(--color-surface-border-rgb))] px-4 py-2.5 text-sm font-medium text-[rgb(var(--color-on-surface-rgb))] transition-colors duration-200 hover:bg-[rgb(var(--color-surface-alt-2-rgb))]">
                        <BugAntIcon /><span>{t('settings.support.reportIssue')}</span>
                    </a>
                    <p className="text-center text-xs text-[rgb(var(--color-on-surface-faint-rgb))]">{t('settings.support.reportDesc')}</p>
                </div>
            </div>
        </div>
    );
