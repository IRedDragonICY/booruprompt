...
// Utility functions
const utils = {
  // Determines tag category from element class
  getCategoryFromClassList: (element: Element): TagCategory => {
    const classMap: Record<string, TagCategory> = {
      'tag-type-copyright': 'copyright',
      'tag-type-character': 'character',
      'tag-type-general': 'general',
      'tag-type-metadata': 'meta',
      'tag-type-artist': 'other'
    };

    for (const [className, category] of Object.entries(classMap)) {
      if (element.classList.contains(className)) return category;
    }

    return 'general';
  },

  // Determines tag category from header text
  ...
  // Extracts tags using class-based approach (Danbooru, Gelbooru, e621)
  extractTagsByClass: (doc: Document, selectors: { container: string, tag: string }): ExtractedTag[] => {
    ...
    elements.forEach(element => {
      ...
      if (tagName) {
        ...
        const cleanName = tagName.replace(/ \(\d+\.?\d*[kM]?\)$/, '');
  ...
  // Extracts tags using section headers (Safebooru, Rule34)
...
const extractionStrategies = {
  danbooru: (doc: Document) => utils.extractTagsByClass(doc, {
    container: '#tag-list li.flex',
    tag: 'a.search-tag'
  }),

  safebooru: (doc: Document) => utils.extractTagsBySection(doc, '#tag-sidebar', 'a:nth-of-type(2)'),

  gelbooru: (doc: Document) => utils.extractTagsByClass(doc, {
    container: '.tag-list-container li',
    tag: 'a'
  }),

  rule34: (doc: Document) => utils.extractTagsBySection(doc, '#tag-sidebar', 'a:nth-child(2)'),

  e621: (doc: Document) => utils.extractTagsByClass(doc, {
...
// CORS proxy options
const CORS_PROXIES = [
  { name: 'AllOrigins', createUrl: (url: string) => `https://api.allorigins.win/raw?url=${encodeURIComponent(url)}` },
  { name: 'CORS Anywhere', createUrl: (url: string) => `https://cors-anywhere.herokuapp.com/${url}` },
  { name: 'CORS.sh', createUrl: (url: string) => `https://cors.sh/?${encodeURIComponent(url)}` },
  { name: 'None (direct fetch)', createUrl: (url: string) => url }
];
...
// UI Components
const CategoryToggle = ({ category, count, onToggle }: {
...
const BooruTagExtractor = () => {
  ...
  const [proxyIndex, setProxyIndex] = useState(0); // AllOrigins is default
  ...
  const handleCopy = () => {
    try {
      navigator.clipboard.writeText(displayedTags)
          .then(() => {
            setCopySuccess(true);
  ...
  const extractTags = useCallback(async () => {
    ...
    try {
      ...
      const proxyUrl = proxy.createUrl(url);
      ...
      // Extract tags using site-specific strategy
      const extractedTags = site.extractTags(doc);
      ...
      setAllExtractedTags(extractedTags);
      ...
      <div className="min-h-screen bg-gray-900 text-gray-100 flex items-center justify-center p-4 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-gray-800 via-gray-900 to-black">
        <div className="w-full max-w-lg bg-gray-800 rounded-lg overflow-hidden border border-gray-700 shadow-xl transform transition-all duration-500 hover:shadow-2xl animate-fadeIn">
          ...
          <div className="p-6 space-y-6 animate-fadeIn animation-delay-200">
            ...
            {/* Proxy selector with animated highlight */}
            <div className="transform transition-all duration-300 hover:scale-[1.01]">
              <label htmlFor="proxy" className="block text-sm font-medium text-gray-300 mb-2">
                CORS Proxy
              </label>
              <select
                  id="proxy"
                  className="w-full bg-gray-700 border border-gray-600 rounded-md p-3 text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-300"
                  value={proxyIndex}
                  onChange={(e) => setProxyIndex(parseInt(e.target.value))}
              >
                {CORS_PROXIES.map((proxy, index) => (
                    <option key={index} value={index} className={index === 0 ? 'text-blue-300 font-medium' : ''}>
                      {proxy.name}
                    </option>
                ))}
              </select>
          ...
          <div className="border-t border-gray-700 p-4 bg-gray-750 text-gray-400 text-xs relative overflow-hidden">
            ...
            <p className="relative z-10">Last updated: 2025-03-03 08:42:29 | Created with <span className="animate-heartBeat inline-block">❤️</span> by <a href="https://x.com/ireddragonicy" className="underline hover:text-blue-400 transition-colors">IRedDragonICY</a></p>
          </div>
        </div>
      </div>
  );
